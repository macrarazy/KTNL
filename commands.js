...
var commands = exports.commands = {
	...
	kick: function(target, room, user) {
		if (!target) return this.sendReply('/kick [user] - Kick user from the room you are in.');
		if (user.locked || user.mutedRooms[room.id]) return this.sendReply("You cannot do this while unable to talk.");
		...
		//targetUser.send('|c|~|/kick ' + target);
		return connection.sendTo(target, "|noinit|nonexistent|You were kicked from the room by '" + user.name + "'." + (target ? " (" + target + ")" : ""));
		this.add('|unlink|' + this.getLastIdOf(targetUser));
	},
